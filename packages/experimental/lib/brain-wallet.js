...
var BrainWallet = /** @class */ (function (_super) {
    ...
    BrainWallet._generate = function (username, password, legacy, progressCallback) {
        ...
        if (typeof (username) === 'string') {
            ...
            usernameBytes = boa_ethers_1.ethers.utils.toUtf8Bytes(username.normalize('NFKC'));
        }
        else {
            usernameBytes = boa_ethers_1.ethers.utils.arrayify(username);
        }
        if (typeof (password) === 'string') {
            logger.checkNormalize();
            passwordBytes = boa_ethers_1.ethers.utils.toUtf8Bytes(password.normalize('NFKC'));
        }
        else {
            passwordBytes = boa_ethers_1.ethers.utils.arrayify(password);
        }
        return scrypt_js_1.default.scrypt(passwordBytes, usernameBytes, (1 << 18), 8, 1, 32, progressCallback).then(function (key) {
            ...
            var mnemonic = boa_ethers_1.ethers.utils.entropyToMnemonic(boa_ethers_1.ethers.utils.arrayify(key).slice(0, 16));
            return new BrainWallet(boa_ethers_1.ethers.Wallet.fromMnemonic(mnemonic));
        });
    };
